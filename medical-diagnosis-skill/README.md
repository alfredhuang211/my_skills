...
✅ **渐进式问诊** - 2-3个问题一组，避免信息过载，用户体验友好  
✅ **逐步鉴别诊断** - 分3轮排除感冒、鼻窦炎、血管运动性鼻炎  
✅ **智能决策树** - 根据回答动态决定下一步问题，最少10题，最多23题  
✅ **精准评分** - TNSS评分系统，分2轮询问，清晰明了  
✅ **个性化方案** - 根据患者特征自动调整治疗方案  
✅ **安全用药** - 自动识别禁忌症和药物相互作用  
✅ **危险预警** - 实时检测危险信号，及时提示就医  
✅ **随访管理** - 分组评估症状改善，智能推荐用药调整  
✅ **购药指导** - 分2组收集偏好，智能匹配需求，生成购买链接  
...
├── scripts/
│   └── ar_diagnosis.py               # 诊断算法Python实现（含鉴别诊断）
├── references/
...
**第三步：排除血管运动性鼻炎**
- 评估：诱因、鼻痒程度、过敏原
- 标准：冷空气/气味诱发 + 无明显鼻痒 + 无过敏原 → 疑似血管运动性鼻炎

**第四步：确认过敏性鼻炎**
- 必须满足：打喷嚏 + 清涕 + 鼻痒 + 鼻塞（≥2个）+ 反复发作 + 有过敏原
